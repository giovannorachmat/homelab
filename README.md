...
| **OCR Loader**     | build `services/ocr-loader/`                                          | internal only                | 2 / 1G           | Open WebUI document-loader shim: tika first, `glm-ocr` on ollama for scanned PDFs/images | ✅ Running    |
...
| **Paperless**      | `ghcr.io/paperless-ngx/paperless-ngx` + valkey + gotenberg            | 8000                         | 1.75 / 2.6G      | Document management; post-consume `ocr_rescue.py` re-OCRs empty scans via `glm-ocr` | ✅ Running    |
